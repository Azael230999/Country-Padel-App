...
export function HoyScreen({ isAdmin, students, eventos, nav, setNav, onVerPagos }) {
  const hoy = new Date();
      ...
      <div style={styles.content} className="content-safe">
        ...
        <div style={styles.card}>
          ...
          <div style={{ fontSize: 12, color: COLORS.muted, marginTop: 4 }}>
            {pendientes.length === 0 ? "todos al día" : `alumno${pendientes.length !== 1 ? "s" : ""} con pago pendiente`}
          </div>
          {pendientes.length > 0 && (
            <button
              type="button"
        ...
        <div style={styles.card}>
          ...
          {eventosHoy.length === 0 && <div style={{ fontSize: 12.5, color: COLORS.muted, textAlign: "center", padding: "10px 0" }}>Sin eventos programados hoy.</div>}
          {eventosHoy.map((e) => (
...
export function PagosPendientesScreen({ students, onBack, onSelectAlumno }) {
  ...
  return (
    <>
      ...
      <div style={styles.content} className="content-safe">
        {pendientes.length === 0 && <div style={styles.empty}>Todos tus alumnos están al día.</div>}
